  ...
  return (
    <div className={`group grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-white/[0.055] ${active ? "bg-white/[0.07]" : ""}`}>
      ...
      <div className="flex items-center gap-3 text-xs text-[#7f877e]">
        ...
        {track.storeUrl && <a aria-label={`Open ${track.title} on Spotify`} href={track.storeUrl} target="_blank" rel="noreferrer" className="opacity-0 transition-opacity hover:text-white group-hover:opacity-100"><MoreHorizontal className="h-4 w-4" /></a>}
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  const playTrack = (track: Track) => {
    if (!track.audio) {
      toast.info("This track has no preview. Open the linked Spotify track to listen there.");
      if (track.storeUrl) window.open(track.storeUrl, "_blank", "noopener,noreferrer");
      return;
    }
